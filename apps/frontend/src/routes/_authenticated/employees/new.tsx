...
function NewEmployeePage() {
  ...
  const renderSelect = (name: string, label: string, items: { code: string; label: string }[]) => (
    <div className="space-y-2">
      <Label>{label}</Label>
      <Select value={form.watch(name) ?? ""} onValueChange={(v) => form.setValue(name, v)}>
        <SelectTrigger>
  ...
  const renderInput = (name: string, label: string, type = "text", placeholder?: string) => (
    <div className="space-y-2">
      <Label>{label}</Label>
      <Input
        type={type}
