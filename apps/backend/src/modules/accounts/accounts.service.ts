...
export async function create(
  ...
  if (existingLink) throw new ConflictError("Nhân sự đã được liên kết với một tài khoản khác");
...
export async function update(
  ...
  if (data.status && data.status !== existing.status) {
    if (id === actorUserId && data.status === "locked") {
  ...
  // Revoke sessions when account is locked
  if (data.status === "locked" && data.status !== existing.status) {
