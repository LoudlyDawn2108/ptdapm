import type { CreateAccountInput, PaginatedResponse, UpdateAccountInput } from "@hrms/shared";
...
export async function create(
  ...
  const [employee] = await db
    .select({ id: employees.id, staffCode: employees.staffCode, fullName: employees.fullName })
    .from(employees)
  ...
  if (existingLink) throw new ConflictError("Nhân sự này đã có tài khoản liên kết");
  ...
  if (existingEmail) {
    throw new FieldValidationError({ email: "Email đã được sử dụng" });
  }
...
export async function update(
  ...
  await withAuditLog(db, actorUserId, "UPDATE", "auth_user", id, oldValues, newValues);
...
export async function setStatus(
  ...
  await db.update(authUsers).set({ status, updatedAt: new Date() }).where(eq(authUsers.id, id));
